...
RoutingProtocol::AddHeaders (Ptr<Packet> p, Ipv4Address source, Ipv4Address destination, uint8_t protocol, Ptr<Ipv4Route> route)
{
  ...
  UdpHeader udpHeader;
  p->RemoveHeader (udpHeader);
  uint32_t port_des_now = udpHeader.GetDestinationPort();

  if (port_des_now != GPSR_PORT) 
    {
                             ...
                             UnicastForwardCallback ucb, ErrorCallback ecb)
{
  ...
  if (!tHeader.IsValid ())
    {
      NS_LOG_DEBUG ("Forwarding-GPSR message " << p->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
      return false;     // drop
    }
  if (tHeader.Get () == GPSRTYPE_POS)
    {
  ...
  if(inRec){
    p->AddHeader (hdr);
    p->AddHeader (tHeader); 
    p->AddHeader (udpHeader); //put headers back so that the RecoveryMode is compatible with Forwarding and SendFromQueue
                              ...
                              Ptr<NetDevice> oif, Socket::SocketErrno &sockerr)
{
  ...
  if (nextHop != Ipv4Address::GetZero ())
    {
      Vector relayPos = Vector (1, 0, 0);
     
      relayPos = m_locationService->GetPosition (nextHop);
